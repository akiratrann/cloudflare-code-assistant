...
import { t as applyChunkToParts } from "./message-builder-Bwc_Umye.js";
...
function extractClientToolSchemas(tools) {
	warnDeprecated("extractClientToolSchemas", "extractClientToolSchemas() is deprecated. Define tools on the server and use onToolCall for client execution. Will be removed in the next major version.");
...
function useAgentChat(options) {
	...
	if (tools) warnDeprecated("useAgentChat.tools", "The 'tools' option in useAgentChat is deprecated. Define tools on the server using tool() from 'ai' and handle client execution via the onToolCall callback. Will be removed in the next major version.");
	...
	const toolsRequiringConfirmation = useMemo(() => manualToolsRequiringConfirmation ?? (tools ? detectToolsRequiringConfirmation(tools) : []), [manualToolsRequiringConfirmation, tools]);
	...
	*/
	...
	const customTransport = useMemo(() => new WebSocketChatTransport({
	...
	*/
	...
	useEffect(() => {
		...
		*/
		...
		if (resume) agent.send(JSON.stringify({ type: MessageType.CF_AGENT_STREAM_RESUME_REQUEST }));
		return () => {
